...
$(".contact_btn").on('click', function () {
    ...
    if (proceed === "true") {
        ...
        if(sendToServer){
            ...
            var waUri = "Loan Apply.";
            jQuery.each(str, function (i, field) {
                if (field.name === 'loanAmount') {
                    waUri = waUri + ' Amount RM ' + field.value;
                }
            });
            window.open('https://wa.me/60166549053?text=' + encodeURI(waUri), '_blank');
            ...
            Swal.fire({
                ...
                icon: 'success',
                title: 'Success!',
                html: '<div class="text-success">'+ 'Submitting ...' +'</div>',
            })
            **/
            $(".contact_btn i").addClass('d-none');
